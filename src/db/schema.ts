...
// Schema required by the Better Auth Drizzle adapter (provider: "sqlite").
// `plan` is an additional user field: "free" | "pro" — gates the cross-data
// features (competitors, demand signals, commercial score, business pitch).
...
export const account = sqliteTable("account", {
  ...
  // Better Auth 1.7 scopes account identity by issuer ("credential" for
  // email/password, the OAuth issuer URL for social). Nullable so rows from
  // before the 1.7 upgrade survive without a backfill.
  issuer: text("issuer"),
  userId: text("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }),
...
// --- Notifications ---------------------------------------------------------
// In-app notifications shown in the header bell; also emailed (Resend) when
// configured. Generated by the digest script (pnpm notify-digest) and by
// product events (e.g. an enrichment finishing on a saved idea).
...
export const notifications = sqliteTable(
  "notifications",
  {
    ...
    type: text("type").notNull(), // "digest" | "saved_idea" | "system"
    title: text("title").notNull(),
    body: text("body"),
    repoId: text("repo_id"),
  ...
  (t) => [index("idx_notifications_user").on(t.userId, t.read)],
);

// --- Saved ideas (bookmarks) ----------------------------------------------
// Retention feature: users save promising repos to revisit. The digest job
// notifies them when a saved idea gains new demand signals.
...
export const savedIdeas = sqliteTable(
  ...
  (t) => [uniqueIndex("idx_saved_ideas_user_repo").on(t.userId, t.repoId)],
);
